...
class _JumpScreenState extends State<JumpScreen> {
  ...
  void _startJumpCounter() {
    ...
    // Set up the settings service for dynamic thresholds
    final settingsService = Provider.of<JumpDetectionSettingsService>(context, listen: false);
    _jumpDetector.setSettingsService(settingsService);
      ...
      if (_newJumps > 0) {
        ...
        if (_hasPremiumAccess && _sessionStartTime != null) {
          try {
            ...
            print('✅ Calories recorded for session: $_newJumps jumps, ${sessionDuration.toStringAsFixed(1)} minutes');
          } catch (e) {
        ...
        if (mounted) {
          ...
          // Navigate back with the total count
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('No new jumps to save.'),
